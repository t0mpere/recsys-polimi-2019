...
class MatrixFactorization_BPR_Theano(object):
    ...
    def _generate_train_model_function(self):
        ...
        self.W = theano.shared(np.random.random((self.n_users, self.rank)).astype('float32'), name='W')
        self.H = theano.shared(np.random.random((self.n_items, self.rank)).astype('float32'), name='H')

        # Bias
        self.B = theano.shared(np.zeros(self.n_items).astype('float32'), name='B')
    ...
    def fit(self, train_data, epochs=30, batch_size=1000):
        """
        ...
        train_data = np.array(list(zip(train_data.nonzero()[0], train_data.nonzero()[1])))
        print(train_data)
    ...
    def _uniform_user_sampling(self, n_samples):
        ...
        sgd_users = np.array(list(self.train_users))[np.random.randint(len(list(self.train_users)), size=n_samples)]
        sgd_pos_items, sgd_neg_items = [], []
        for sgd_user in sgd_users:
            pos_item = self.train_dict[sgd_user][np.random.randint(len(self.train_dict[sgd_user]))]
    ...
    def get_expected_ratings(self, user_index):
        """
    ...
    def get_expected_rating(self, user_index, item_index):
        """
          Predicts the preference of a given `user_index`
          for a gven `item_index`.
        """
        return self.get_expected_ratings(user_index)[item_index]

    def recommend(self, user_index, topn=10):
        """
        ...
        """
        return [
            item_index for item_index in np.argsort(self.get_expected_ratings(user_index))
    ...
    def test(self, test_data):
        ...
        for user in test_dict.keys():
            if user in self.train_users:
                auc_for_user = 0.0
                n = 0
                predictions = self.get_expected_ratings(user)
...
if __name__ == '__main__':
    ...
    recommender = MatrixFactorization_BPR_Theano(100, urm.shape[0], urm.shape[1])
    Runner.run(recommender, True, split='2080')
